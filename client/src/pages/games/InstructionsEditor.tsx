...
export const InstructionsEditor: FC<{
  ...
  const addInstructionsMutation = useAddGameInstructionsMutation(existingGame?.id)
  ...
  const gettingStartedControl = useTextInput(existingGame?.gettingStartedInstructions ?? "")

  const submitHandler = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    addInstructionsMutation.mutateAsync({ rules: rulesControl.value, gettingStarted: gettingStartedControl.value })
      .then(() => navigate("/games"))
  };
  return (
