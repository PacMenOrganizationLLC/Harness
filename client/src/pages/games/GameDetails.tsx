...
import { useGetEndpointTypesQuery, useGetGameEndpointsQuery } from "./endpoints/endpointHooks"
...
export const GameDetails: FC<{
  ...
  const requiredEndpointTypes = useMemo(() => endpointTypesQuery.data ? endpointTypesQuery.data.filter(t => t.required === true) : undefined, [endpointTypesQuery.data])
  const gameEndpointsQuery = useGetGameEndpointsQuery(selectedGame.id)
  const requiredEndpoints = useMemo(() => gameEndpointsQuery.data ? gameEndpointsQuery.data.filter(e => e.endpointType?.required === true) : undefined, [gameEndpointsQuery.data])
  const deleteGameMutation = useDeleteGameMutation();
  const navigate = useNavigate();
  const needsEndpoints = requiredEndpointTypes && requiredEndpoints && requiredEndpointTypes.length > requiredEndpoints.length
  ...
  return (
    <div className="col-md-8 col border rounded shadow-sm p-3" key={selectedGame.id}>
      ...
      <button className="btn btn-primary mt-2"
        onClick={() => navigate(`/endpoints/${selectedGame.id}`)}>Configure Endpoints</button>
